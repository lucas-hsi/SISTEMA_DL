...
interface AnunciosLayoutProps {
  children: React.ReactNode;
}

const AnunciosLayout = ({ children }: AnunciosLayoutProps) => {
  ...
  useEffect(() => {
    const checkAccess = () => {
      ...
      // LÓGICA HIERÁRQUICA: Permitir acesso se for 'anuncios' OU 'gestor'
      const allowedRoles = ['anuncios', 'gestor'];
      const currentRole = user?.role;
      
      if (!currentRole || !allowedRoles.includes(currentRole)) {
        // Se não for anúncios nem gestor, redirecionar para login
        router.push('/login');
        return;
      }
